...
def create_input_bean(input_file_bean):
    ...
    if input_file_bean.frame_specification_flag:
        ...
        if number_utils.is_less(input_file_bean.start_frame, 1):
            raise BusinessError(error_id.E0000003, 'start_frame' ,'1' , input_file_name)
        
        # フレーム数は1以上を指定
        if number_utils.is_less(input_file_bean.end_frame, 1):
            raise BusinessError(error_id.E0000003, 'end_frame' ,'1' , input_file_name)
        
        # === 複合チェック ===
        # ビデオストリームがない場合、フレーム指定不可
        if video_stream_bean is None:
            raise BusinessError(error_id.E0000008, input_file_name)
        
        # 開始フレームは総フレーム数以下を指定
        if number_utils.is_greater(input_file_bean.start_frame, video_stream_bean.nb_frames):
            raise BusinessError(error_id.E0000004, 'start_frame', str(video_stream_bean.nb_frames), input_file_name)
        
        # 終了フレームは総フレーム数以下を指定
        if number_utils.is_greater(input_file_bean.end_frame, video_stream_bean.nb_frames):
            raise BusinessError(error_id.E0000004, 'end_frame', str(video_stream_bean.nb_frames), input_file_name)
        
        # 終了フレームは開始フレーム以上を指定
        if number_utils.is_less(input_file_bean.end_frame, input_file_bean.start_frame):
            raise BusinessError(error_id.E0000003, 'end_frame' ,str(input_file_bean.start_frame) , input_file_name)
        ...
        if number_utils.is_less(input_file_bean.start_time, 0):
            raise BusinessError(error_id.E0000003, 'start_time' ,'0' , input_file_name)
        
        # 切り取り期間は0以上を指定
        if number_utils.is_less(input_file_bean.trim_duration, 0):
            raise BusinessError(error_id.E0000003, 'trim_duration' ,'0' , input_file_name)
        
        # === 複合チェック ===
        # 開始フレームは動画再生時間以下を指定
        if number_utils.is_greater(input_file_bean.start_time, format_bean.duration):
            raise BusinessError(error_id.E0000004, 'start_frame', str(format_bean.duration), input_file_name)
        ...
        if number_utils.is_greater(end_time, format_bean.duration):
            raise BusinessError(error_id.E0000004, 'start_frameとtrim_durationの合計', str(format_bean.duration), input_file_name)
        # ====================
